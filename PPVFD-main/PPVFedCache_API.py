...
class PPVFedCache_standalone_API:
    ...
    def do_ppvfedcache_stand_alone(self, client_models, train_data_local_num_dict, test_data_local_num_dict,
        ...
        for global_epoch in range(args.comm_round):
            ...
            # wandb.log({f"Attack Success Rate": float(np.mean(np.array(attack_success_rate)))}, step=global_epoch)
            # print("client_connection:{}".format(client_connection))
            sample_connect={}
            relation_origin = knowledge_cache.output_relation()
            # print(relation_origin[(0,0)])
            ...
            # print("关系:{}".format(sample_connect[0,0]))
            # print(local_knowledge.keys())
            # self.relation={
            # "(client_idx,idx)": [(client_idx,idx),(client_idx,idx),(client_idx,idx)]
            # "(client_idx,idx)":[(client_idx,idx),(client_idx,idx),(client_idx,idx)]
            # }
            ...
            #      }
            # }
            # total_sum = torch.zeros_like(local_knowledge[0,0])
            # for i in sample_connect[0,0]:
            #     print(local_knowledge[i])
            #     total_sum += local_knowledge[i]
            # print("求和：{}".format(total_sum/30))

            oracle = lcc_cache.ALCCOracle(
                X=local_knowledge,
                ...
                fit_intercept=False,
                args=args,
                train_data_local_num_dict=train_data_num,
                sample_connect=sample_connect,
                cache=knowledge_cache.output_cache()
            )
            ...
            # 获取每个客户端接收到的数据
            # print("接收数据")
            # received_encoded_knowledge = oracle.encoded_knowledge_distribute(local_encoded_knowledge)
            # del local_encoded_knowledge
            # total_memory = sum(value.nbytes for value in received_encoded_knowledge.values())
            # print("内存占用：{}".format(total_memory))
            # 每个客户端执行本地计算 要上传的logits
            # print("本地聚合")
            # fun_encoded_knowledge = oracle.worker_fun(received_encoded_knowledge)
            # del received_encoded_knowledge
            # 中心服务器接收数据进行解码计算、聚合、签名计算等 更新global_knowledge并把相关的信息发送给本地客户端。
            # print("开始解码")
            # Y_decoded_global_knowledges, self.sum_k= oracle.ALcc_decoded(fun_encoded_knowledge)
            # del fun_encoded_knowledge
            debug_global_knowledge, debug_global_sum_knowledge = oracle.debug_ALcc()
            # print("sum_k:{}".format(self.sum_k[(0,0)]))
            # print("debug_sum_k:{}".format(debug_global_sum_knowledge[(0,0)]))
            # summed_loss_1 = lcc_cache.rel_err_batch(Y_decoded_global_knowledges, debug_global_knowledge,args.bat_num)
            # summed_loss_2 = lcc_cache.rel_err(self.sum_k, debug_global_sum_knowledge)
            # print("summed_loss_2:{}".format(np.mean(summed_loss_2)))
            # wandb.log({str(global_epoch) + " Lcc_Loss": np.mean(summed_loss_2)}, step=global_epoch)
            # wandb.log({f"mean Loss on all clients": float(np.mean(np.array(summed_loss_2)))},
            #            step=global_epoch)

            for idx in global_knowledge.keys():
                # global_knowledge[idx] = torch.Tensor(np.real(self.sum_k[idx]))
                global_knowledge[idx] = torch.Tensor(np.real(debug_global_sum_knowledge[idx]))
